...
export async function executePlan(
  ...
  for (const action of plan) {
    ...
    try {
      await withTimeout(
        executeAction(action, client, context),
        action.action === 'navigate' ? NAV_TIMEOUT_MS : ACTION_TIMEOUT_MS,
        `action "${action.action}" step ${action.step}`
      );
      ...
      if (action.selector) {
        try {
          ...
          await withTimeout(
            executeViaCSSSelector(action, client),
            ACTION_TIMEOUT_MS,
            'selector fallback'
          );
...
async function clickNode(action: PlannedAction, client: any, context: PageContext): Promise<void> {
  if (action.nodeId) {
    ...
    if (!nodeId) throw new Error(`Could not resolve nodeId for backend ${node.backendDOMNodeId}`);

    // Scroll element into view
    await client.DOM.scrollIntoViewIfNeeded({ nodeId }).catch(() => {});

    const { model } = await client.DOM.getBoxModel({ nodeId });
    if (!model) throw new Error('getBoxModel returned no model');
...
async function selectOption(action: PlannedAction, client: any, context: PageContext): Promise<void> {
  if (!action.nodeId) throw new Error('select requires nodeId');
  ...
  const nodeId = nodeIds[0];
  if (!nodeId) throw new Error('Could not resolve DOM nodeId');
...
async function clickViaJS(selector: string, client: any): Promise<void> {
  ...
  if (exceptionDetails) throw new Error(exceptionDetails.text || 'clickViaJS failed');
}

async function waitForDOMSettle(client: any): Promise<void> {
  let stableCount = 0;
  let lastHtml = '';
