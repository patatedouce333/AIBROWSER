...
export async function generatePlan(
  ...
  const response = await callMercury(SYSTEM_PROMPT, userMessage, { apiKey, temperature: 0.1, maxTokens: 4096 });
  ...
  try {
    ...
    const corrected = await callMercury(
      CORRECTION_PROMPT,
      `Original response:\n${response}\n\nFix it and return only the JSON array.`,
      { apiKey, temperature: 0, maxTokens: 4096 }
    );
...
function parseAndValidate(raw: string, nodeMap: Map<number, any>): PlannedAction[] {
  ...
  if (!Array.isArray(steps)) throw new Error('Response is not a JSON array');
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    if (s.text) step.text = String(s.text);
    if (s.url) step.url = String(s.url);
    if (s.direction) step.direction = s.direction === 'up' ? 'up' : 'down';
    if (s.amount) step.amount = Number(s.amount);
    if (s.condition) step.condition = String(s.condition);
    if (s.timeoutMs) step.timeoutMs = Number(s.timeoutMs);
    if (s.pressEnter !== undefined) step.pressEnter = Boolean(s.pressEnter);

    valid.push(step);
  }

  if (valid.length === 0) throw new Error('Plan is empty after validation');
  return valid;
}
